...
class AttParser:
    ...
    def __init__(self, api: ParserApi, pause: int = 60, clear: bool = False) -> None:
        ...
        self.api = api
        self.db = None
        self.task_instance = None  # Ссылка на Celery задачу для проверки остановки
        self.manual_stop = False  # Режим ручной остановки
    ...
    async def _init_kucoin_parsers(self, miss, last_launch, time_parser, count, *input_args):
        ...
        if isinstance(self.api, KuCoinAPI):
            if miss:
                logger.warning("Missed data not implemented for KuCoinAPI, ignoring miss parameter")
    ...
    async def parse(self, count: int = 10, miss: bool = False,
                    ...
                    manual_stop: bool = False, *input_args) -> dict[str, pd.DataFrame]:
    ...
    async def start_manager_kucoin(self, check_stop, func_parser, time_parser, count, *args):
        # Инициализируем словарь coins из coin_list
        coins = {}
        if self.coin_list:
            # Если есть список монет, инициализируем их
    ...
    def _check_stop(self, *args, **kwargs):
        # Проверяем, не была ли задача остановлена через Celery
        if self.task_instance is not None:
            # Проверяем статус задачи через request
            if hasattr(self.task_instance, 'request') and hasattr(self.task_instance.request, 'is_aborted'):
                if self.task_instance.request.is_aborted():
                    logger.info("Task was aborted by user")
                    return False
            
            # Проверяем, не была ли задача отозвана (через request.revoked)
            if hasattr(self.task_instance, 'request') and hasattr(self.task_instance.request, 'revoked'):
                if self.task_instance.request.revoked:
                    logger.info("Task was revoked")
                    return False
        
        # Если режим ручной остановки, продолжаем до тех пор, пока задача не будет остановлена
        if self.manual_stop:
            return True
        
        # Для обычного режима проверяем count
        count = kwargs.get('count', -1)
        if count == -1:
            return True
        
        all_dataframes = kwargs.get('all_dataframes', {})
        if not all_dataframes:
            return True
        
        # Проверяем, достигли ли мы нужного количества для всех монет
        # Для упрощения возвращаем True, чтобы продолжать парсинг
        return True
